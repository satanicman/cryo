...
class FOOTERCONTACTS extends Module
{
	public function __construct()
	{
		$this->name = 'footercontacts';
		$this->description = 'contact information in footer';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Footercontacts_MONOLANGUAGE';
		$this->text_editor_content = 'Footercontacts_TEXTEDITOR';
		$this->content_wrapper = 'Footercontacts_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Footercontacts_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Footercontacts_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('footerContacts');
		$this->description = $this->l('contact information in footer');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !FOOTERCONTACTSModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !FOOTERCONTACTSModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = FOOTERCONTACTSModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = FOOTERCONTACTSModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					FOOTERCONTACTSModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					FOOTERCONTACTSModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																FOOTERCONTACTSModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class FOOTERCONTACTSModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( FOOTERCONTACTSModel::createFilesTable()
				&& FOOTERCONTACTSModel::createContentTable());
	}
