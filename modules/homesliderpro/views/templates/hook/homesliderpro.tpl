...
{if isset($homeslider_slides) && $homeslider_slides|@count > 0}
		<ul id="slider" class="slider index" style="margin:0;">
		{assign "number" "0"} {* short-hand *}
		{foreach from=$homeslider_slides item=slide}
			{$number = $number+1}
			{if $slide.active && $slide.image != ''}
				<li title="{$slide.description|escape:'htmlall':'UTF-8'}">
					{if $slide.url != ''}
						<a class="SElink" href="{$slide.url|escape:'htmlall':'UTF-8'}" {if $slide.new_window == 1}target="_blank"{/if}>
					{/if}
                    <div class="min-slider-img">
						<img class="SEimage" src="{$smarty.const._MODULE_DIR_}/homesliderpro/images/{$slide.image|escape:'htmlall':'UTF-8'|replace:'resize_':''}" alt="{$slide.legend|escape:'htmlall':'UTF-8'}"/>
                    </div>
					{if $slide.url != ''}
						</a>
					{/if}
					{if $slide.description != ''}
						<p class="min-slider-description">{$slide.description}</p>
					{/if}
